<article id="post-<?php the_ID(); ?>" <?php post_class( 'clearfix' ); ?> role="article" itemscope
	...
	<footer class="clearfix">
		<?php wp_link_pages( array(
			'before' => '<nav class="page-navigation">' . __( 'Pages:', 'lightly' ),
			'after'  => '</nav>',
		) ); ?>
		...
		if ( ! $disable_category_meta_box && 'post' == get_post_type() ): ?>
			<div id="article-footer-meta">
				<p><?php _e( 'Posted in ', 'lightly' );
					the_category( ', ' ); ?>. <?php the_tags( __( 'Tagged as ', 'lightly' ), ', ', '' ); ?></p>
			</div>
		<?php endif; ?>
		...
		if ( ! $disable_post_nav_box && 'post' == get_post_type() ): ?>
			<nav class="post-nav clearfix">
				<?php next_post_link( '%link', '<span class="meta">' . __( 'Next Post &rarr;', "lightly" ) . '</span> <h3 class="post-title-small">%title</h3>' ); ?>
				<?php previous_post_link( '%link', '<span class="meta">' . __( '&larr; Previous Post', "lightly" ) . '</span> <h3 class="post-title-small">%title</h3>' ); ?>
			</nav>
		<?php endif; ?>
		...
		if ( ! $disable_author_box && 'post' == get_post_type() ): ?>
			<div id="author-box">
				<h3 class="widgettitle"><span><?php _e( 'Author', 'lightly' ); ?></span></h3>
				...
				<?php if ( $user_url = get_the_author_meta( 'user_url' ) ) : ?>
					<p class="meta"><a
							href="<?php echo esc_url( $user_url ); ?>"><?php _e( 'Author Website', 'lightly' ); ?></a></p>
				<?php endif; ?>
		...
		if ( ! $disable_related_box && 'post' == get_post_type() ): ?>
			<div id="related-box">
				<h4 class="widgettitle"><span><?php _e( 'Related Posts', 'lightly' ); ?></span></h4>
