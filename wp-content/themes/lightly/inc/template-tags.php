...
function lightly_comments( $comment, $args, $depth ) {
	...
	<article id="comment-<?php comment_ID(); ?>" class="clearfix">
		...
		<?php if ( $comment->comment_approved == '0' ) : ?>
			<div class="help">
				<p><?php _e( 'Your comment is awaiting moderation.', 'lightly' ) ?></p>
			</div>
		<?php endif; ?>
...
function lightly_breadcrumb() {
	if ( ! is_front_page() ) {
		echo '<div id="breadcrumbs" class="col940"> <a href="' . esc_url( home_url() ) . '">' . __( 'Home', 'lightly' ) . '</a> ';
	}
	...
	if ( is_tag() ) {
		printf( __( 'Tag: %s', 'lightly' ), single_tag_title( '', false ) );
	}
	if ( is_404() ) {
		_e( '404 - Page not Found', 'lightly' );
	}
	if ( is_search() ) {
		_e( 'Search', 'lightly' );
	}
	...
	if ( is_author() ) {
		printf( __( 'Posts by %s', 'lightly' ), get_the_author() );
	}
...
function lightly_related_posts() {
	...
	if ( $related_posts ) :
		...
		<div class="no_related_post type-3"><?php _e( 'Cannot Retrieved a Related Posts Yet!', 'lightly' ); ?></div>
	<?php endif;
