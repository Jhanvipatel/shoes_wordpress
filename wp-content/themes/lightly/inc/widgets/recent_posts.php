...
class Lightly_Widget_Recent_Posts extends WP_Widget {
	...
	function __construct() {
		parent::__construct( 'lightly_recent_posts', __( 'Lightly - Recent Posts', 'lightly' ), array(
			'classname'   => 'widget_posts_wrap',
			'description' => __( 'Display most recent Posts.', 'lightly' )
		) );
	}

	function widget( $args, $instance ) {
		$default  = array(
			'title'    => __( 'Recent Posts', 'lightly' ),
			'cats'     => '',
			'cat'      => '',
			'quantity' => 5,
			'order'    => 'date',
	...
	public function form( $instance ) {
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Widget title (Will automaticly use category name when select single category):', 'lightly' ); ?></label><br/>
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'cat' ) ); ?>"><?php _e( 'Category:', 'lightly' ); ?></label><br/>
			<select id="<?php echo esc_attr( $this->get_field_id( 'cat' ) ); ?>"
			        name="<?php echo esc_attr( $this->get_field_name( 'cat' ) ); ?>">
				<option
					value="0" <?php selected( 0, $cat ); ?>><?php _e( 'Multiple Categories', 'lightly' ); ?></option>
				<?php
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'cats' ) ); ?>"><?php _e( 'Enter ID of categories e.g. 1,2,3,4. Leave it blank to pull all categories (if multiple category choosed).:', 'lightly' ); ?></label><br/>
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'quantity' ) ); ?>"><?php _e( 'Number of posts:', 'lightly' ); ?></label><br/>
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"><?php _e( 'Order posts by:', 'lightly' ); ?></label><br/>
			<select id="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"
			        name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>">
				<option
					value="date" <?php selected( 'date', $order ); ?>><?php _e( 'date', 'lightly' ); ?></option>
				<option
					value="rand" <?php selected( 'rand', $order ); ?>><?php _e( 'random', 'lightly' ); ?></option>
				<option
					value="comment_count" <?php selected( 'comment_count', $order ); ?>><?php _e( 'popular', 'lightly' ); ?></option>
			</select>
		</p>
		<?php
	}
