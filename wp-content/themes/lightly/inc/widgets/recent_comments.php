...
class Lightly_Widget_Recent_Comments extends WP_Widget {
	...
	function __construct() {
		parent::__construct( 'lightly_recent_comments', __( 'Lightly - Recent Comments', 'lightly' ), array(
			'classname'   => 'widget_comments_wrap',
			'description' => __( 'Display most recent comments.', 'lightly' ),
		) );
	}

	function widget( $args, $instance ) {
		$default  = array(
			'title'    => __( 'Latest Comments', 'lightly' ),
			'quantity' => 5
		);
	...
	public function form( $instance ) {
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Widget title:', 'lightly' ); ?></label><br/>
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->get_field_id( 'quantity' ) ); ?>"><?php _e( 'Number of Comments:', 'lightly' ); ?></label><br/>
